      ...
      <div className={`min-w-0 flex flex-col ${variant === 'grid' ? 'order-last p-4' : variant === 'short' ? 'flex-1 justify-between' : ''}`}>
        <div className="flex items-start justify-between gap-4">
          ...
          <div ref={menuRef} className={`${variant === 'grid' ? 'absolute right-3 top-3 z-20' : 'relative shrink-0'}`}>
            ...
            {menuOpen && <div ref={actionMenuRef} id={menuId} role="menu" tabIndex={-1} aria-label={`Options for ${post.title}`} onKeyDown={handleMenuKeyDown} className="absolute right-0 top-10 z-[80] w-64 overflow-hidden rounded-[12px] border border-[var(--color-border)] bg-[var(--color-surface)] p-1 shadow-[var(--shadow-menu)]">
