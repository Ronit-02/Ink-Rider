...
- The root [dockerfile](../dockerfile) uses Node 20 and `npm install`, while the root README and CI use Node 22+. It copies built frontend files into `/app/public`, but static serving is commented out in [app.js](../Backend/src/app.js) and no SPA fallback exists. That image does not currently serve the full frontend. Prefer independent static hosting plus an API service; pin a supported compatible Node runtime and lockfile-based installation.
...
- [app.js](../Backend/src/app.js) has no `trust proxy` configuration. Validate the actual hosting proxy chain before relying on `req.ip` for rate limits and vote-abuse signals. Broadly trusting arbitrary forwarded headers is unsafe. [Express proxy configuration](https://expressjs.com/en/guide/behind-proxies/).
...
[rate-limit.middleware.js](../Backend/middlewares/rate-limit.middleware.js) keeps buckets in a process-local Map. Before running multiple API instances, use a shared rate-limit store or equivalent centralized enforcement that preserves account-level rules. Redis is a reasonable candidate; it is not currently installed or required just to launch one instance.
